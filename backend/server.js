...
app.post('/poem', authenticate, async (req, res) => {
    console.log('poem page')
    // res.send(req.rootUser)

    try {
        const { email, poem } = req.body;
        if (!poem) {
            console.log('message is empty server')
            return res.json({ error: "fill the message input" })
        }
        ...
        if (userContact) {
            const userMessage = await userContact.addPoem(email, poem)
            await userContact.save();
            res.status(201).json({
                message: "user contacted successfully"
            })
        ...
        console.log(` poem server error - ${err}`)
    }
})
...
app.post('/updateProfile', async (req, res) => {
    ...
    try {
        ...
        console.log(`err in updateProfile server try -- ${err}`)
    }
})
